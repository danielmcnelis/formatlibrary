...
export const generateNewToken = async (server) => {
  ...
  try {
    ...
    console.log('installed.client_secret', installed.client_secret)
    console.log('installed.client_id', installed.client_id)

    const { data } = await axios.post('https://www.googleapis.com/oauth2/v4/token', {
      "installed.client_id": installed.client_id,
      "installed.client_secret": installed.client_secret,
...
export const uploadDeckFolder = async (server, tournamentName, decks) => {
  ...
  try {
    ...
    const folder = `./decks/${server.name}/${tournamentName}`
