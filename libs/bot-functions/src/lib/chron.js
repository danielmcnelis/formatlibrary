...
export const runFrequentTasks = async (client) => {
    ...
    return setTimeout(() => runFrequentTasks(client), 1 * 60 * 1000)
}
...
export const runSomewhatFrequentTasks = async (client) => {
    ...
    return setTimeout(() => runSomewhatFrequentTasks(client), 5 * 60 * 1000)
}
...
export const runNightlyTasks = async (client) => {
    try {
        ...
        const tasks = [
            purgeEntries, purgeTournamentRoles, assignTournamentRoles,
    ...
    for (let i = 0; i < pools.length; i++) {
        ...
        for (let i = 0; i < potentialPairs.length; i++) {
            try {
                ...
                //     (pool.createdAt < fiveMinutesAgo && potentialPair.createdAt < fiveMinutesAgo && Math.abs(yourElo - potentialPairElo) <= 133) ||
                //     (pool.createdAt < tenMinutesAgo && potentialPair.createdAt < tenMinutesAgo && Math.abs(yourElo - potentialPairElo) <= 167) ||
                //     (pool.createdAt < fifteenMinutesAgo && potentialPair.createdAt < fifteenMinutesAgo && Math.abs(yourElo - potentialPairElo) <= 200)
                // )) {
...
export const recalculateFormatStats = async (format) => {
    ...
    const servers = await Server.findAll({
        ...
        attributes: ['id', 'name', 'hasInternalLadder', 'formatId']
    })
    ...
    for (let z = 0; z < servers.length; z++) {
        ...
        const filteredMatches = await allMatches.filter((e) => e.winnerId === 'fwHShAv8MNkPxK5f6PWt3H' && !!e.isSeasonal && e.createdAt >= '2026-06-24 00:00:00+00')
...
// UPDATE DECKTYPE SUMMARIES
export const updateDeckTypeSummaries = async () => {
    const start = Date.now()
    const chronRecord = await ChronRecord.create({
