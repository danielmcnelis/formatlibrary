...
import { Iron, Player, Stats } from '@fl/models'
...
export const initiateIron = async (channel, format) => {
    ...
    return setTimeout(async () => {
        ...
        if (count) {
            ...
            const missingNames = missingEntries.map((entry) => entry.player.globalName)
...
export const postStory = async (channel, format) => {
    ...
    return setTimeout(() => postPairing(channel, format), 5000)
}
