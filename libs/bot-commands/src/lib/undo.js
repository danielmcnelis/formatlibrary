...
export default {
    ...
    async execute(interaction) {
        ...
        if (!winnerStats.backupElo) return await interaction.editReply({ content: `Your last opponent, ${winningPlayer.globalName || winningPlayer.discordName}, has no backup stats. Please get a Moderator to help you.`})
        ...
        await match.destroy()
        return await interaction.editReply({ content: `The last ${server.internalLadder ? 'Internal ' : ''}${format.name} Format ${server.emoji || format.emoji} ${match.isTournament ? 'Tournament ' : ''}match in which ${winningPlayer.globalName || winningPlayer.discordName}${tournament?.pointsEligible ? ` (-${match.round + 1}) TP)` : ''} defeated ${losingPlayer.globalName || losingPlayer.discordName}${tournament?.pointsEligible && match.round === 1 ? ` (-1 TP)` : ''} has been erased.`})	
    }
}
