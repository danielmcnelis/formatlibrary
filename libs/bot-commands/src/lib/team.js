...
export default {
    ...
    async execute(interaction) {
        ...
        if (captain.hidden) return await interaction.reply({ content: `You are not allowed to play in Format Library sanctioned tournaments.`})
        const teammate1 = await Player.findOne({ where: { discordId: interaction.options.getUser('teammate1').id }})
        if (teammate1.hidden) return await interaction.reply({ content: `${teammate1.name} is not allowed to play in Format Library sanctioned tournaments.`})
        const teammate2 = await Player.findOne({ where: { discordId: interaction.options.getUser('teammate2').id }})
        if (teammate1.hidden) return await interaction.reply({ content: `${teammate2.name} is not allowed to play in Format Library sanctioned tournaments.`})
