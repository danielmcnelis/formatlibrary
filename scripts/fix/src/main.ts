import { Artwork, Alius, Card, Community, Cube, Deck, DeckType, DeckThumb, Entry, Event, Format, Match, Membership, Pairing, Player, Pool, Print, Replay, Ruling, Server, Set, Stats, Status, Team, Tournament, BlogPost, Matchup } from '@fl/models'
    ...
    for (let i = 0; i < events.length; i++) {
        try {
            ...
            if (!blogpost) {
                ...
                const freqs = decks.reduce((acc, curr) => (acc[curr.deckTypeName] ? acc[curr.deckTypeName]++ : acc[curr.deckTypeName] = 1, acc), {})
                const popularDecks = Object.entries(freqs).sort((a:any, b:any) => b[1] - a[1]).map((e) => e[0]).slice(0, 6)
